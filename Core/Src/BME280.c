...
void Trim_Read(void)
{
	uint8_t trimdata[32]; //see page no 24 there are only 32 bytes
	//read NVC from 0x88 to 0xA1
...
int BME280_config(uint8_t osrs_t,uint8_t osrs_p, uint8_t osrs_h,uint8_t mode,uint8_t t_sb,uint8_t filter)
{
	...
	//write humidity oversampling values to 0xF2 register
	datatoWrite = osrs_h;
	...
	HAL_Delay(100);
	HAL_I2C_Mem_Read(BME280_I2C, BME280_ADDRESS, CTRL_HUM_REG, 1, &datatoCheck, 1, 1000);
	...
	HAL_Delay (100);
	HAL_I2C_Mem_Read(BME280_I2C, BME280_ADDRESS, CONFIG_REG, 1, &datatoCheck, 1, 1000);
	if (datatoCheck != datatoWrite)
	{
		return -1;
	}

	//Temp and pressure register setting 0xF4;
	datatoWrite= (osrs_t<<5)| (osrs_p<<2)|mode;
	if(HAL_I2C_Mem_Write(BME280_I2C, BME280_ADDRESS,CTRL_MEAS_REG,1,&datatoWrite,1, 1000)!= HAL_OK){
		return -1;
	}
	HAL_Delay(100);
	HAL_I2C_Mem_Read(BME280_I2C,BME280_ADDRESS,CTRL_MEAS_REG, 1, &datatoCheck,1,1000);
	if(datatoCheck != datatoWrite){
	return -1;
	}
...
void BME280_WakeUP(void){
	...
	//enabling the forced mode
	datatoWrite |= Forced;
	//writing the data to CTRL_Meas reg to enable forced mode
	HAL_I2C_Mem_Write(BME280_I2C, BME280_ADDRESS, CTRL_MEAS_REG,1,&datatoWrite, 1, 1000);
	HAL_Delay(100);
}

int BME280_ReadRaw(void)
{
	uint8_t RawData[8];
